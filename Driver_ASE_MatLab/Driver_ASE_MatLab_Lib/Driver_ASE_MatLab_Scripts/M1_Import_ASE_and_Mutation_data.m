function [All_Assoc,top_mut_dsd]=M1_Import_ASE_and_Mutation_data(cancer_type,regex4feature,varargin)
...
fdr_cutoff=0.2;
if (nargin==3)
    fdr_cutoff=varargin{1};
    if (ischar(fdr_cutoff))
        %need to make it as double when compiling the function
        %with applicationcompiler.
        fdr_cutoff=str2double(fdr_cutoff);
    end
end

scriptdir = pwd;
%can not use mfilename when using applicationcompiler to make mcr
%executable matlab function.
%as the real compiled matlab function would be included in a temporary dir;
%fileparts(which(mfilename)); %get directory path of exexuting function
...
disp('load somatic_calls/tx2gene2coding.lookup.txt')
somatic_call_file='somatic_calls/tx2gene2coding.lookup.txt';
disp(somatic_call_file)
[look.tx,look.gene,look.coding] = textread(somatic_call_file,'%s%s%n','delimiter','\t');
...
disp('Current dir:')
disp(pwd)
disp('Going to load mut_ase_look.txt')
[mut_ase_look.num,mut_ase_look.name,mut_ase_look.type] = textread('mut_ase_look.txt','%n%s%s','delimiter','\t');
...
d = ExtrAssocFromTopMutDSD(top_mut_dsd,'anno_type','.*','fdr',fdr_cutoff);
...
outfile=strcat('driver_top_fdr',num2str(fdr_cutoff),'.txt');
OutPutDriverParametersIntoTxt(top,outfile);
